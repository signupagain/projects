<script setup lang="ts">
	...
	const { nodeStack, layout, searchValue, sortBy, order } =
		storeToRefs(dataStore)

	const breadcrumbItems = computed<
		(BreadcrumbItem & { children?: DropdownMenuItem[] })[]
	>(() => {
		const items = nodeStack.value.map(
			(route, index) =>
	...
	<header class="border-default border-b-2 p-4 sm:px-6 lg:px-8">
		...
		<div class="mb-4 flex items-center justify-between">
			<UButton
				...
				@click="backTo(nodeStack.length - 2)"
			/>
			<UBreadcrumb
			...
			>
				<template #item="{ item }">
					<UButton
						...
						:color="
							(
								item.index === nodeStack.length - 1 &&
								(item.index !== 0 || nodeStack.length > 3)
							) ?
								'primary'
							:	'neutral'
						"
						:disabled="item.index === nodeStack.length - 1"
						@click="backTo(item.index)"
						>{{ item.label }}</UButton
					>
				</template>
				<template #dropdown>
